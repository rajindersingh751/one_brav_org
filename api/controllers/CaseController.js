...
module.exports = {
  ...
  getMediatorProfiles: (req, res) => { // Later put limit of 50
    mediatorService.getAllMediatorPublicProfiles((ok, docs) => {
      if (ok) {
        res.json({
          ok: true,
          message: 'Here are all mediators',
          list: docs
        });
      } else {
        res.json({
  ...
  createMediationSession: (req, res) => {
    ...
    if (!!req.body.mediatorsArray) {
      ...
      res.json({
        ok: false,
        message: 'Missing params : mediatorsArray'
      });
      return;
    }
    if (toReturn) {
    ...
    mediatorService.getMediatorMiniProfilesForMediationSession(mediatorsArray, (mediatorsLookup) => {
      if (mediatorsLookup.ok) {
        ...
        res.json({
          ok: false,
          message: 'issue in retriving mediators list'
        })
      }
    });
  },
  ...
  getAllMediationSessionRequests: (req, res) => {
    MediationSessionService.getMediationSessionRequests({
      yes: false
    }, req.authDecoded.email, req.authDecoded.type, (resObject) => {
      res.json(resObject);
    });
  },
  ...
  respondToMediationRequest: (req, res) => {
    ...
    if (req.body.id && req.body.accept) {
      ...
      res.json({
        ok: false,
        message: 'missing params : id or accept'
      })
    }
  },
  ...
  actionsToMediationRequest: (req, res) => {
    ...
    if (req.body.id && req.body.action && req.body.data) {
      ...
      res.json({
        ok: false,
        message: 'missing params : id or action and data'
      })
    }
  },
  ...
  payNow: (req, res) => {
    if (req.body.trxnId && req.body.stripeToken && req.body.amount) {
      ...
      MediationSessionService.getMediationSessionRequests({
        yes: true,
        sessionId: sessionId
      }, req.authDecoded.email, req.authDecoded.type, resultObject => {
        if (resultObject.ok) {
          ...
          MediationSessionService.makePayment(
            ...
            resObject => {
              if (resObject.ok) {
                ...
                res.json({
                  ok: false,
                  message: 'Payment Failed due to resObject',
                  resObj: resObject
                });
              }
            });
  ...
  getAllMediationSessions: (req, res) => {
    MediationSessionService.getMediationSessions({
      yes: false
    }, req.authDecoded.email, req.authDecoded.type, (resObject) => {
      res.json(resObject);
    });
